...
provider_class = Puppet::Type.type(:aws_igw).provider(:api)

describe provider_class do
  context "with 2 resources in each of 2 regions in 2 accounts" do
    let(:two_igws) {[:igw1, :igw2]}
    ...
    let(:ec2_mock) {
      ...
      ec2_mock.stub_chain('regions.[].internet_gateways').and_return(two_igws)
      ec2_mock
    }
    ...
    before :each do
      ...
      expect(provider_class).to receive(:new_from_aws) do |a1, a2|
        [:igw1, :igw2].include?(a1).should be(true)
