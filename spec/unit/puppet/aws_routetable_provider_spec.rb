...
provider_class = Puppet::Type.type(:aws_routetable).provider(:api)

describe provider_class do
  context "with 2 resources in each of 2 regions in 2 accounts" do
    let(:two_routetables) {[:routetable1, :routetable2]}
    ...
    let(:ec2_mock) {
      ...
      ec2_mock.stub_chain('regions.[].route_tables').and_return(two_routetables)
      ec2_mock
    }
    ...
    before :each do
      ...
      expect(provider_class).to receive(:new_from_aws) do |a1, a2, a3|
        ...
        [:routetable1, :routetable2].include?(a2).should be(true)
